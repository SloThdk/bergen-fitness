...
const TRAINERS = [
  {
    name: 'Erik Hansen',
    role: 'Head Trainer · HIIT & Boxing',
    bio: '10 years experience. Former Norwegian boxing champion. Specialises in high-intensity functional training.',
  ...
  },
  {
    ...
    bio: 'Certified in Hatha and Vinyasa yoga. Expert in mindful movement and core rehabilitation.',
  ...
  },
  {
    ...
    bio: 'CrossFit Level 3 Trainer. Powerlifting background. Helping athletes build raw strength and endurance.',
  ...
  {
    name: 'Sara Moe',
    role: 'Spin & Cardio Specialist',
    bio: 'Indoor cycling certified, 7 years coaching competitive cyclists and recreational riders.',
...
export default function BergenFitness() {
  ...
  const handleBook = (className: string) => {
    setBookingMsg(`✓ Booking requested for "${className}" — demo only`);
  ...
  return (
    <div style={{ background: 'var(--navy)', color: '#fff', minHeight: '100vh' }}>
      {/* NAVBAR */}
      <nav style={{ background: 'rgba(13,27,42,0.95)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <button style={{ background: 'var(--orange)', color: '#fff', borderRadius: '6px', padding: '8px 20px', fontSize: '14px', fontWeight: 600 }}
        ...
        {menuOpen && (
          <div style={{ background: 'var(--navy-mid)', borderTop: '1px solid rgba(255,255,255,0.06)' }} className="md:hidden px-6 py-4 flex flex-col gap-4">
            {NAV_LINKS.map(link => (
              <a key={link} href={`#${link.toLowerCase()}`} style={{ color: 'rgba(255,255,255,0.8)', fontSize: '15px' }}
                onClick={() => setMenuOpen(false)}>{link}</a>
            ))}
            <button style={{ background: 'var(--orange)', color: '#fff', borderRadius: '6px', padding: '10px', fontSize: '14px', fontWeight: 600 }}>
              Join Now
            </button>
          </div>
        )}
      ...
      <section className="relative min-h-screen flex items-center" style={{ paddingTop: '64px' }}>
        <div className="absolute inset-0 overflow-hidden">
          <Image
            ...
            alt="Bergen Fitness gym interior"
            fill
            style={{ objectFit: 'cover', objectPosition: 'center' }}
            priority
          />
          <div style={{ background: 'linear-gradient(105deg, rgba(13,27,42,0.95) 45%, rgba(13,27,42,0.5) 100%)' }}
            className="absolute inset-0" />
          <div style={{ background: 'linear-gradient(to top, rgba(13,27,42,1) 0%, transparent 40%)' }}
            className="absolute inset-0" />
        </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 py-24">
          <div className="max-w-2xl">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span style={{ color: 'var(--orange-light)', fontSize: '13px', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase' }}>
                Bergen&apos;s Premier Gym
              </span>
            </div>
            <h1 style={{ fontFamily: 'var(--font-syne)', fontSize: 'clamp(48px, 7vw, 88px)', fontWeight: 800, lineHeight: 1.0, letterSpacing: '-0.03em' }}>
              Train Harder.<br />
              <span style={{ color: 'var(--orange)' }}>Live Stronger.</span>
            </h1>
            ...
            <div className="flex flex-wrap items-center gap-4 mt-10">
              <button style={{ background: 'var(--orange)', color: '#fff', borderRadius: '8px', padding: '14px 32px', fontSize: '16px', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '8px' }}
                className="hover:opacity-90 transition-opacity">
                Start Free Trial <ArrowRight size={18} />
              </button>
              <button style={{ border: '1px solid rgba(255,255,255,0.25)', color: '#fff', borderRadius: '8px', padding: '14px 28px', fontSize: '16px', fontWeight: 500 }}
      ...
      <section id="schedule" className="py-24">
        <div className="max-w-7xl mx-auto px-6">
          <RevealSection>
            <div className="flex items-start justify-between flex-wrap gap-4 mb-12">
              <div>
                ...
                <h2 style={{ fontFamily: 'var(--font-syne)', fontSize: 'clamp(32px, 4vw, 48px)', fontWeight: 800 }}>
                  Find Your<br />Perfect Class
                </h2>
              </div>
          ...
          <RevealSection>
            <div className="flex flex-wrap gap-2 mb-8">
              {CLASSES.map((day, i) => (
                <button key={day.day} onClick={() => setActiveDay(i)}
                  style={{
                    background: activeDay === i ? 'var(--orange)' : 'var(--navy-mid)',
                ...
                return (
                  <div key={i}
                    style={{
                      ...
                      padding: '16px 24px', flexWrap: 'wrap', gap: '12px',
                      borderBottom: i < CLASSES[activeDay].schedule.length - 1 ? '1px solid rgba(255,255,255,0.06)' : 'none',
                    }}>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <button
                        onClick={() => !isFull && handleBook(cls.name)}
                        style={{
                          background: isFull ? 'transparent' : 'var(--orange)',
            ...
            {bookingMsg && (
              <div style={{ marginTop: '12px', background: 'rgba(232,93,4,0.15)', border: '1px solid rgba(232,93,4,0.3)', borderRadius: '8px', padding: '12px 16px', color: 'var(--orange-light)', fontSize: '14px' }}>
                {bookingMsg}
              </div>
            )}
          </RevealSection>
      ...
      <section id="membership" style={{ background: 'var(--navy-mid)', padding: '96px 0' }}>
        <div className="max-w-7xl mx-auto px-6">
          <RevealSection className="text-center mb-16">
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '16px', marginTop: '12px', maxWidth: '480px', margin: '12px auto 0' }}>
              No contracts. Cancel anytime. First 7 days always free.
            </p>
          </RevealSection>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {PLANS.map((plan, i) => (
                <div key={plan.name}
                  ...
                  style={{
                    background: plan.highlight ? 'var(--orange)' : 'var(--navy)',
                    border: activePlan === i && !plan.highlight ? '2px solid var(--orange)' : plan.highlight ? 'none' : '1px solid rgba(255,255,255,0.08)',
                    borderRadius: '16px', padding: '32px', cursor: 'pointer',
                    transform: plan.highlight ? 'scale(1.03)' : 'scale(1)',
                    boxShadow: plan.highlight ? '0 20px 60px rgba(232,93,4,0.35)' : 'none',
                    transition: 'all 0.2s',
                  }}
                >
                  {plan.highlight && (
                    <div style={{ background: 'rgba(255,255,255,0.2)', borderRadius: '20px', padding: '4px 12px', fontSize: '11px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', display: 'inline-block', marginBottom: '16px' }}>
                      Most Popular
                    </div>
                  )}
                  ...
                  <ul style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '28px' }}>
                    {plan.features.map(f => (
                      <li key={f} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '14px', color: plan.highlight ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.7)' }}>
                        <Check size={15} color={plan.highlight ? '#fff' : 'var(--orange)'} strokeWidth={3} />
                        {f}
                      </li>
                    ))}
                  </ul>
                  <button style={{
                    ...
                    background: plan.highlight ? '#fff' : 'var(--orange)',
                    color: plan.highlight ? 'var(--orange)' : '#fff',
        ...
        <div className="max-w-7xl mx-auto px-6">
          <RevealSection className="mb-14">
            ...
            <div className="flex items-end justify-between flex-wrap gap-4">
              <h2 style={{ fontFamily: 'var(--font-syne)', fontSize: 'clamp(32px, 4vw, 48px)', fontWeight: 800 }}>
                World-Class<br />Trainers
              </h2>
              <a href="#" style={{ color: 'var(--orange)', fontSize: '14px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {TRAINERS.map((trainer, i) => (
              <RevealSection key={trainer.name}>
                <div style={{
                  background: 'var(--navy-mid)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px', overflow: 'hidden',
                  ...
                  <div style={{ padding: '16px' }}>
                    ...
                    <button style={{ marginTop: '14px', width: '100%', padding: '9px', border: '1px solid rgba(232,93,4,0.4)', borderRadius: '6px', color: 'var(--orange)', fontSize: '13px', fontWeight: 600 }}
                      className="hover:bg-orange-500/10 transition-colors">
                      Book Session <span className="demo-badge" style={{ marginLeft: '6px', fontSize: '9px', padding: '2px 6px' }}>Demo</span>
                    </button>
                  </div>
                </div>
              </RevealSection>
      ...
      {/* CTA SECTION */}
      <section style={{ background: 'var(--orange)', padding: '80px 0' }} id="contact">
        <div className="max-w-5xl mx-auto px-6 text-center">
          <RevealSection>
            <h2 style={{ fontFamily: 'var(--font-syne)', fontSize: 'clamp(32px, 5vw, 56px)', fontWeight: 800, lineHeight: 1.1 }}>
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 mt-10">
              <button style={{ background: '#fff', color: 'var(--orange)', borderRadius: '8px', padding: '14px 32px', fontSize: '16px', fontWeight: 700 }}
                className="hover:opacity-90 transition-opacity">
                Start Free Trial
              </button>
              <button style={{ border: '2px solid rgba(255,255,255,0.5)', color: '#fff', borderRadius: '8px', padding: '14px 28px', fontSize: '16px', fontWeight: 600 }}
      ...
      <footer style={{ background: 'var(--navy-mid)', borderTop: '1px solid rgba(255,255,255,0.06)', padding: '48px 0 32px' }}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-4 gap-10 mb-12">
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span style={{ fontFamily: 'var(--font-syne)', fontWeight: 800, fontSize: '16px' }}>
                  Bergen<span style={{ color: 'var(--orange)' }}>Fitness</span>
                </span>
              </div>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px', lineHeight: 1.7 }}>
                Bergen&apos;s premier fitness destination. Open 24/7, 365 days a year.
              </p>
            ...
            <div>
              ...
              {[
                { icon: MapPin, text: 'Bryggen 14, Bergen' },
                { icon: Phone, text: '+47 555 01 234' },
                { icon: Mail, text: 'info@bergenfitness.no' },
              ].map(({ icon: Icon, text }) => (
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '24px', color: 'rgba(255,255,255,0.25)', fontSize: '12px', textAlign: 'center' }}>
            © 2025 BergenFitness · Fictional demo site built for Fiverr portfolio
          </div>
        </div>
      </footer>
    </div>
